...
[旧商品データ]テーブルから[新商品データ]テーブル、[カテゴリーデータ]テーブルにデータを移行するため、Spring Batch を使用して移行バッチを作成する。Spring Batch はバッチ処理を行うためのフレームワークで、バッチ処理で必要になる多くの便利な機能を提供しておりそれを利用する。インターネット等で調べ、Spring Batch の使い方を把握する
...
また実際には実装してみないと見えてこない部分もあるかと思うので、ある程度のラフに設計をおこない、実装しながら修正していくという流れでも構わない
...
上述した[要件](./docs/feature/create_category/README.md)に、各テーブルの定義があるので、それに従ってDDLを作成しテーブルを作成する
...
### 5.移行バッチの実装
「3.」の設計に基づき、Spring Batch を使用して移行バッチを実装する。本テンプレートリポジトリーから自身のアカウントにリポジトリーを作成（複製）し、そこで作業を進める。やり方は以下を参照
